...
try:
    from jev_decisions import classify_with_jev as _classify_with_jev
except Exception:
    _classify_with_jev = None


def simple_classify(X):
    # Try Jev first (confidence-routed); fall back to original on None
    if _classify_with_jev is not None:
        jev_result = _classify_with_jev(X)
