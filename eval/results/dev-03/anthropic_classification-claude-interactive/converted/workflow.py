...
def simple_classify(X):
    # Try Jev Choice first; fall back to original on rejection/error
    jev_label = None
    try:
        from jev_decisions import evaluate_classify
        jev_label, accepted = evaluate_classify(X)
    except Exception:
        accepted = False

    if accepted and jev_label is not None:
        return jev_label
